...
_pad_right() {
  ...
  [[ $padding -gt 0 ]] && printf "%s%*s" "$str" "$padding" "" || echo "$str"
}
...
_print_result_line() {
  ...
  local http_col=$(_pad_right "${http_code}" 6)
  ...
  elif [[ "$http_code" == "FAIL" || "$http_code" == "TIMEOUT" ]]; then
    local fail_col=$(_pad_right "失败" 6)
    printf "%b%s%b%s%b%s%b%s %s %s%s%b%s%b\n" \
      "$RED" "$num_col" "$NC" "$name_col" "$RED" "$fail_col" "$NC" "$ttft_col" "$total_col" "$model_col" "$token_col" "$RED" "$reply" "$NC"
  else
    printf "%b%s%b%s%b%s%b%s %s %s%s%b%s%b\n" \
...
show_status() {
  ...
  if [[ ! -f "$CLAUDE_SETTINGS" ]]; then
    printf "%b错误：未找到配置文件 %s%b\n" "$RED" "$CLAUDE_SETTINGS" "$NC"
    return 1
  fi
...
switch_provider() {
  ...
  if [[ -n "$CUSTOM_MODEL" ]]; then
    printf "%b正在切换到：%s (模型：%s)%b\n" "$CYAN" "$P_NAME" "$CUSTOM_MODEL" "$NC"
  else
    printf "%b正在切换到：%s%b\n" "$CYAN" "$P_NAME" "$NC"
  fi
  echo ""
  ...
  if [[ -n "${CLAUDECODE:-}" ]]; then
    printf "%b提示：当前在 Claude Code 会话内，配置已更新。%b\n" "$YELLOW" "$NC"
    printf "%b请退出当前会话后重新运行 claude 生效。%b\n" "$YELLOW" "$NC"
    echo ""
    return 0
  fi

  printf "%b正在启动 Claude Code...%b\n" "$GREEN" "$NC"
  echo ""
...
" 2>/dev/null)
  ...
  printf "%b╔════════════════════════════════════════════════════════╗%b\n" "$CYAN" "$NC"
  printf "%b║         Claude Code 供应商切换工具                     ║%b\n" "$CYAN" "$NC"
  printf "%b╚════════════════════════════════════════════════════════╝%b\n" "$CYAN" "$NC"
  echo ""
  echo "请选择一个供应商:"
  echo ""
